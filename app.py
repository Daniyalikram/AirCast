...
with tab_explore:
    if clean is None:
        ...
        if bundle is None or features is None:
            ...
            cd = pd.DataFrame({"time": pd.to_datetime(tte.values),
                               "Actual": yte.values,
                               "Predicted (24h ahead)": pred})
            cd_long = cd.melt("time", var_name="series", value_name="pm25")
            line = (alt.Chart(cd_long).mark_line(strokeWidth=0.7).encode(
                        x=alt.X("time:T", title="Date"),
                        y=alt.Y("pm25:Q", title="PM2.5 (µg/m³)"),
                        color=alt.Color("series:N", title="",
                            scale=alt.Scale(domain=["Actual", "Predicted (24h ahead)"],
                                            range=["#34495e", "#e67e22"]))))
            who = (alt.Chart(pd.DataFrame({"y": [WHO_GUIDELINE]}))
                   .mark_rule(color="green", strokeDash=[5, 5]).encode(y="y:Q"))
            st.altair_chart(line + who, use_container_width=True)
    ...
    if bundle is None:
        ...
        try:
            ...
            if fc is None or fc.empty:
                ...
                fc_plot = fc.copy()
                fc_plot["forecast_time"] = pd.to_datetime(fc_plot["forecast_time"])
                line = (alt.Chart(fc_plot).mark_line(color="#e67e22", strokeWidth=2, point=True)
                        .encode(
                            x=alt.X("forecast_time:T", title="Time"),
                            y=alt.Y("predicted_pm25:Q", title="Predicted PM2.5 (µg/m³)",
                                    scale=alt.Scale(zero=True)),
                            tooltip=[alt.Tooltip("forecast_time:T", title="Time"),
                                     alt.Tooltip("predicted_pm25:Q", title="PM2.5")]))
                who = (alt.Chart(pd.DataFrame({"y": [WHO_GUIDELINE]}))
                       .mark_rule(color="green", strokeDash=[5, 5]).encode(y="y:Q"))
                st.altair_chart(line + who, use_container_width=True)
                st.caption("Green dashed line: WHO 24-h guideline (15 µg/m³). "
